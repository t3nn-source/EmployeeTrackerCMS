...
async function viewEmployeesByDepartment(departmentId) {
  try {
    const result = await pool.query('SELECT * FROM employees WHERE department_id = $1;', [departmentId]);
    return result.rows;
  } catch (err) {
...
async function viewDepartmentBudget(departmentId) {
  try {
    const result = await pool.query('SELECT SUM(salary) FROM roles WHERE department_id = $1;', [departmentId]);
    return result.rows[0];
  } catch (err) {
